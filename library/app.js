...
function autoSlider() {
  ...
  choosenProduct = products[slideIndex];
  currentProductTitle.textContent = choosenProduct.title;
  currentProductPrice.textContent = "RM" + choosenProduct.price;
  currentProductDesc.textContent = choosenProduct.desc
  currentProductImg.src = choosenProduct.colors[0].img;
}
let slideInterval = setInterval(autoSlider, 7500);

// call out all buy now button in booklist
const bookListButtons = document.querySelectorAll(".book-list-button");
...
bookListButtons.forEach(button => {
    button.addEventListener("click", () => {
        ...
        currentProductPrice.textContent = "RM" + selectedProduct.price;
